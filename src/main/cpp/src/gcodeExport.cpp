...
void GCodeExport::reset()
{
    currentPosition = Point3(0,0,MM2INT(20));
    layer_nr = 0;
    relative_extrusion = false;

    for (int i = 0; i < MAX_EXTRUDERS; ++i)
    {
        extruder_attr[i].is_primed = false;
        extruder_attr[i].is_used = false;
        extruder_attr[i].extruderCharacter = 0;
        extruder_attr[i].filament_area = 0;
        extruder_attr[i].totalFilament = 0;
        extruder_attr[i].currentTemperature = 0;
        extruder_attr[i].waited_for_temperature = false;
        extruder_attr[i].initial_temp = 0;
        extruder_attr[i].retraction_e_amount_current = 0.0;
        extruder_attr[i].retraction_e_amount_at_e_start = 0.0;
        extruder_attr[i].prime_volume = 0.0;
        extruder_attr[i].last_retraction_prime_speed = 0.0;
        extruder_attr[i].fan_number = 0;
        extruder_attr[i].last_e_value_after_wipe = 0.0;
        std::deque<double>().swap(extruder_attr[i].extruded_volume_at_previous_n_retractions);
    }
    ...
    std::vector<Duration>().swap(total_print_times);
    total_print_times = std::vector<Duration>(static_cast<unsigned char>(PrintFeatureType::NumPrintFeatureTypes), 0.0);
