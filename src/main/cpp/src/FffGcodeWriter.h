...
public:
    ...
    /*!
     *  Reset all variables in FffGcodeWriter to their default value
     */
    void reset()
    {
        max_object_height = 0;
        ...
        layer_plan_buffer.reset();

        int i;
        int targetSize = extruder_order_per_layer_negative_layers.size();
        for (i = targetSize - 1; i >= 0; --i)
        {
            std::vector<size_t>().swap(extruder_order_per_layer_negative_layers[i]);
        }
        std::vector<std::vector<size_t> >().swap(extruder_order_per_layer_negative_layers);

        targetSize = extruder_order_per_layer.size();
        for (i = targetSize - 1; i >= 0; --i)
        {
            std::vector<size_t>().swap(extruder_order_per_layer[i]);
        }
        std::vector<std::vector<size_t> >().swap(extruder_order_per_layer);

        targetSize = mesh_order_per_extruder.size();
        for (i = targetSize - 1; i >= 0; --i)
        {
            std::vector<size_t>().swap(mesh_order_per_extruder[i]);
        }
        std::vector<std::vector<size_t> >().swap(mesh_order_per_extruder);

        for (i = 0; i < MAX_EXTRUDERS; ++i)
        {
            extruder_prime_layer_nr[i] = std::numeric_limits<int>::max();
        }

        std::vector<FanSpeedLayerTimeSettings>().swap(fan_speed_layer_time_settings_per_extruder);
    }
